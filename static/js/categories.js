...
(function(){
  async function loadCategories(){
    try{
      ...
      if(!container) return;
      // preserve current category from URL; default to "For You"
      const params = new URLSearchParams(window.location.search);
      const current = params.get('category') || 'For You';
      container.innerHTML = '';

      // ensure "For You" exists (case-insensitive) and appear at front
      const categories = data.slice();
      const hasForYou = categories.some(c => c && c.toLowerCase() === 'for you');
      if(!hasForYou){
        categories.unshift('For You');
      }

      categories.forEach(cat => {
        const a = document.createElement('a');
        a.setAttribute('role','listitem');
        const isActive = (cat && current && String(cat).toLowerCase() === String(current).toLowerCase());
        a.className = 'category-chip' + (isActive ? ' active' : '');
